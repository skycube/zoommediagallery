...
switch($sorting){
	case 0:
		$this->_sql = "SELECT DISTINCT img.id, img.* FROM ".$dbprefix."zoomfiles AS img WHERE hits > 0 ORDER BY hits DESC LIMIT 10";
		break;
	case 1:
		$this->_sql = "SELECT * FROM ".$dbprefix."zoomfiles ORDER BY id DESC LIMIT 10";
		break;
	case 2:
	...
	case 3:
		...
		$this->_sql = "SELECT * FROM ".$dbprefix."zoomfiles WHERE LOWER(descr) LIKE '%$suchstring%' OR LOWER(name) LIKE '%$suchstring%' OR LOWER(filename) LIKE '%$suchstring%' ORDER BY id DESC";
		break;
	case 4:
		$this->_sql = "SELECT * FROM ".$dbprefix."zoomfiles WHERE votesum > 0 ORDER BY votesum/votenum DESC LIMIT 10";
		break;
	default:
...
while($row = mysql_fetch_array($this->_result)){
	...
	echo '<td align="left"><b>'.$row['filename'].'</b><br>';
