...
DHTMLGoodies_menuModel.prototype = {
    ...
    {    	
    	for(var no = 1; no <= this.menuItems.length; no++){
    	    if (this.menuItems[no]) {
...
DHTMLGoodies_contextMenu.prototype = 
{
	...
	__createDivs : function()
	{
		...
		this.menuObject.style.backgroundImage = 'url(\'' + this.imagePath + 'contextmenu_gradient.gif' + '\')';
	...
	__createMenuItems : function()
	{
		...
		this.menuUls = new Array();
		for(var no = 1; no <= this.menuItems.length; no++) {	// Looping through menu items		
		    if (this.menuItems[no]) {
