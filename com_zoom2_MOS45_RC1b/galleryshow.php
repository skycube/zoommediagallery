...
if (!$catid){
	...
	$sql = "SELECT * FROM ".$mosConfig_dbprefix."zoom WHERE subcat_id=0 AND pos=0";
	...
	while ($row = mysql_fetch_array($result))
	{
	 ...
	 if ($zoom->_CONFIG['catImg']){
	 	...
	 	if ($theImage == ""){
	 		...
	 		?>
	 		<td align=left valign="top" width="50%"><a href="index.php?option=com_zoom&Itemid=<?php echo $Itemid;?>&catid=<?php echo $catid;?>&catname=<?php echo $catname;?>&pos=0">
	...
	$TRecord = $database->openConnectionWithReturn("SELECT * FROM ".$mosConfig_dbprefix."zoomfiles WHERE gallery_id=$catid");
	if (!$TRecord) echo "<span class=\"small\">"._ZOOM_NOPICS."</span>";
	$result = @mysql_query("SELECT * FROM ".$mosConfig_dbprefix."zoomfiles WHERE gallery_id=$catid LIMIT $startRow, $zoom->_pageSize");
	...
	<tr>
		<?php
		$sc_sql = "SELECT id, catname, catdescr, pos FROM ".$mosConfig_dbprefix."zoom WHERE subcat_id=$catid ORDER BY catname ASC";
		$sc_result = $database->openConnectionWithReturn($sc_sql);
		$catcount = 0;
		while($subcat = mysql_fetch_array($sc_result)){
			if ($catcount >= $zoom->_CONFIG['columnsno']){
				echo "<td>&nbsp;</td></tr><tr>\n";
				$catcount = 0;
			}
		?>
		<td>
			...
			<img src="components/com_zoom/images/folder.gif" border="0" align="left">:: <?php echo $subcat['catname'];?></a><br>
	...
	//
	$i = 1;
	$sql1 = "SELECT id,filename,hits,descr FROM ".$mosConfig_dbprefix."zoomfiles WHERE gallery_id = $catid ORDER BY id ASC LIMIT $startRow, $zoom->_pageSize";
	...
	while ($row = mysql_fetch_object($result1))
		...
		if($zoom->_CONFIG['commentsOn']){
			...
			$sqlc = 'SELECT date_format( date, \'%d-%m-%y\' ) AS date, name FROM '.$mosConfig_dbprefix.'zoom_comments WHERE image_id = '.$row->id.' ORDER BY date ASC';
