...
# Version:                1b005
# Date:                   2006|12|19
# Requirements:			        com_zoom_2.5.1 RC3 or higher
#
...
#####################################################################################
# Spezial thanks to
# Andreas Mastny (Weilheim, Germany)
# A. Aliu (Swizerland) www.arsimi.ch
#####################################################################################
#
# Per Lasse Baasch
...
else 
{
 ...
 for($i=0; $i<$row0[0]->mycount; $i++)
 {
   ...
   $imgthumb = explode (".",$row2[0]->imgfilename);
   if (($imgthumb[1] != 'jpg') && ($imgthumb[1] != 'JPG'))
		 	$imgthumb[1] = 'jpg';
	  $database->setQuery("SELECT catid,catdir from #__zoom WHERE catid=".$row2[0]->catid." limit 1");
   $row3=$database->loadObjectList();
	  $catimage = "<img style=\"vertical-align: middle;\" src=\"$BaseImagePath".$row3[0]->catdir."/thumbs/".$imgthumb[0].".".$imgthumb[1]."\" border=\"0\" width=\"$zoomMenueImageW\" height=\"$zoomMenueImageH\">";
 	}
	 else
  ...
  $row4=$database->loadObjectList();
   ...
   if($row4[0]->subcount!=0)
   {
     ...
     $row6=$database->loadObjectList();
		   $imgthumb = explode (".",$row6[0]->imgfilename);
 		  if (($imgthumb[1] != 'jpg') && ($imgthumb[1] != 'JPG'))
	 	  	$imgthumb[1] = 'jpg';
     $database->setQuery("SELECT catid,catdir from #__zoom WHERE catid=".$row6[0]->catid." limit 1");
     $row7=$database->loadObjectList();
     $subcatimage = "<img src=\"$BaseImagePath".$row7[0]->catdir."/thumbs/".$imgthumb[0].".".$imgthumb[1]."\" border=\"0\" width=\"$zoomMenueSubImageW\" height=\"$zoomMenueSubImageH\">";
    }
	   else
